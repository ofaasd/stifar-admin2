            ...
            $var['data_mahasiswa'] = DB::table('mahasiswa as m')
                                ...
                                ->addSelect([
                                    'ips' => DB::table('master_nilai as mn')
                                        ...
                                        ->selectRaw('ROUND(SUM((mk.sks_teori + mk.sks_praktek) * mn.kualitas) / NULLIF(SUM(mk.sks_teori + mk.sks_praktek), 0), 2)')
                                        ->whereColumn('mn.id_mhs', 'm.id')
                                        ->where('mn.id_tahun', $id_tahun) // Filter Semester Ini
                                ])
                                ...
                                // 3. Hitung IPK (Kumulatif / Semua Semester)
                                ->addSelect([
                                    'ipk' => DB::table('master_nilai as mn')
                                        ->join('mata_kuliahs as mk', 'mn.id_matkul', '=', 'mk.id')
                                        ->selectRaw('ROUND(SUM((mk.sks_teori + mk.sks_praktek) * mn.kualitas) / NULLIF(SUM(mk.sks_teori + mk.sks_praktek), 0), 2)')
                                        ->whereColumn('mn.id_mhs', 'm.id')
                                        ->whereNotNull('mn.nhuruf') // Hanya yang sudah ada nilainya
                                ])
                                ...
                                // 5. AMBIL SEMUA NILAI SEMESTER INI DALAM FORMAT JSON
                                // Output mentah: [{"id_mk": 101, "huruf": "A"}, {"id_mk": 102, "huruf": "BC"}]
                                ->addSelect([
                                ...
                                ->where('m.status', 1) // Mahasiswa Aktif
                                ->orderBy('m.nim', 'asc')
                                ->get();
